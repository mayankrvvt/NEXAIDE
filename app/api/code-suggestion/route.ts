...
export async function POST(request: NextRequest) {
  try {
    ...
    if (fileContent === undefined || cursorLine === undefined || cursorColumn === undefined || !suggestionType) {
...
function analyzeCodeContext(content: string, line: number, column: number, fileName?: string): CodeContext {
  ...
  return { language, framework, beforeContext, currentLine, afterContext, cursorPosition: { line, column }, isInFunction, isInClass, isAfterComment, incompletePatterns }
}
...
Instructions:
1. Return ONLY raw code, no explanations, no comments, no markdown
2. Do not include any text before or after the code
3. Maintain proper indentation
4. If unsure, return a single relevant code line
Generate suggestion:`
}
...
function detectLanguage(content: string, fileName?: string): string {
  if (fileName) {
    ...
    const extMap: Record<string, string> = { ts: "TypeScript", tsx: "TypeScript", js: "JavaScript", jsx: "JavaScript", py: "Python", java: "Java", go: "Go", rs: "Rust", php: "PHP" }
