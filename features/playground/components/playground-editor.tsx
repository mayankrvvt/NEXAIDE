...
export const PlaygroundEditor = ({
  ...
  const handleEditorDidMount = (editor: any, monaco: Monaco) => {
    ...
    // FIX: Use addAction instead of addCommand for reliable keybinding
    editor.addAction({
      id: "trigger-ai-suggestion",
      label: "Trigger AI Suggestion",
      keybindings: [monaco.KeyMod.CtrlCmd | monaco.KeyMod.Shift | monaco.KeyCode.KeyI],
      run: () => {
        console.log("Cmd+Shift+I pressed, triggering AI suggestion")
        onTriggerSuggestion("completion", editor)
      },
    })

    // CRITICAL: Override Tab key
    ...
    // FIX: Only clear suggestion on cursor move, do NOT auto-trigger
    editor.onDidChangeCursorPosition((e: any) => {
      ...
      // AUTO-TRIGGER REMOVED: AI only triggers on Cmd+Shift+I
    })

    // FIX: Only clear suggestion on typing, do NOT auto-trigger
  ...
  useEffect(() => {
    return () => {
      ...
      if (tabCommandRef.current) {
        if (typeof (tabCommandRef.current as any).dispose === 'function') {
          (tabCommandRef.current as any).dispose()
        }
        tabCommandRef.current = null
      }
    }
